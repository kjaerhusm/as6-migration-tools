...
def add_mapp_package(config_folder_path, subfolders, folder, package_name, package_type):
    """
        ...
        try:                           
            ...
            if objects_node is not None:
                ...
                if not object_elements:
                    ...
                    object_elements = [child for child in objects_node if child.tag.endswith('Object')]
                
                for obj in object_elements:
                    if obj.text == package_name and obj.get('Type') == 'Package':
                        existing_package = obj
                        print(f"{package_name} entry already exists in cpu.pkg in {folder}")
                        break
                ...
                if existing_package is None:
                    ...
                    tree.write(cpu_pkg_path, encoding='utf-8', xml_declaration=True, pretty_print=True)
                    print(f"Added {package_name} object to cpu.pkg in {folder}")
                else:
                    print(f"{package_name} object already exists in cpu.pkg in {folder}")
            else:
        ...
        for folder in physical_folders:
            ...
            if os.path.exists(config_folder_path):     
                ...
                if subfolders:
                    ...
                    mapp_services_exists = add_mapp_package(config_folder_path, subfolders, folder, "mappServices", "mappServices")
                    
                    # Add mappMotion package
                    mapp_motion_exists = add_mapp_package(config_folder_path, subfolders, folder, "mappMotion", "mappMotion")

                    # Add mappView package
                    mapp_view_exists = add_mapp_package(config_folder_path, subfolders, folder, "mappView", "mappViewControl")

        print(f"Please close and reopen the project in Automation Studio to see the changes.")  
